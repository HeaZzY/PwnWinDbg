...
def build_system_prompt(state_summary, code_exec, cmd_cheatsheet):
    ...
    if code_exec:
        python_tool = (
            ...
            "print()).\n"
            "    ⚠ STATE DOES NOT PERSIST BETWEEN ```python BLOCKS. Each block is "
            "a BRAND-NEW process: any variable you set (payload, offset, "
            "win_addr, an ELF/ROP object) is GONE in the next block and using it "
            "raises NameError. So: do a full sub-task in ONE block, or re-derive "
            "/ re-hardcode values from the PRINTED output of the previous block "
            "(if a block printed 'offset 44', the next block must write 44 "
            "literally). The DEBUGGEE and its tube DO persist (they live in the "
            "debugger) — send()/recv()/dbg()/read()/write() keep working across "
            "blocks; only local python variables reset.\n"
